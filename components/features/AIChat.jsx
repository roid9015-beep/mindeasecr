  ...
  return (
    <div style={{ animation:"fadeUp 0.4s ease", padding:"8px 0" }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        <input className="custom-input" placeholder={locale==="es"?"Escribe tu propio tema...":"Write your own topic..."} value={custom} onChange={e=>setCustom(e.target.value)} onKeyDown={e=>e.key==="Enter"&&custom.trim()&&onStart(custom.trim())} />
  ...
  return (
    <div style={{ textAlign:"center", padding:"40px 20px", animation:"fadeUp 0.4s ease" }}>
      ...
      <h3 style={{ fontFamily:"var(--font-main)", fontSize:20, fontWeight:700, marginBottom:10 }}>
        {locale==="es" ? "Tu sesión de este mes ha terminado" : "Your session for this month is complete"}
      </h3>
      <p style={{ color:"var(--text-secondary)", fontSize:14, lineHeight:1.7, maxWidth:320, margin:"0 auto 8px" }}>
        {locale==="es" ? `El plan gratuito incluye 1 conversación profunda por mes. Tu próxima sesión se abre el ${displayDate}.` : `The free plan includes 1 deep conversation per month. Your next session opens on ${displayDate}.`}
      </p>
      <div style={{ display:"flex", flexDirection:"column", gap:10, maxWidth:280, margin:"0 auto", marginTop:24 }}>
        <button className="btn btn-primary" style={{ padding:"14px", fontSize:15 }} onClick={onUpgrade}>
          ⭐ {locale==="es" ? "Actualizar a Premium — $5/mes" : "Upgrade to Premium — $5/month"}
        </button>
      </div>
    </div>
  );
  ...
  if (activeTopic || validMessages.length > 0) {
    return (
      <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 140px)", animation:"fadeUp 0.4s ease" }}>
        ...
        {error && <div style={{ background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:9, padding:"9px 13px", fontSize:13, color:"#f87171", marginTop:8 }}>{error}</div>}
