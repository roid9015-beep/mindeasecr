...
export default function AIChat({ t, locale, countryInfo, user, messages, setMessages, voiceKey, voiceEnabled, sessionLog, startSession, onUpgrade }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Corregido: Tu backend retorna "reply", no "content"
      const aiReply = data.reply || "";
      if (!aiReply) throw new Error("Respuesta vacía de la IA");

      const aiMsg = { role: "assistant", content: aiReply, timestamp: Date.now() };
      setMessages(prev => [...prev, aiMsg]);
      if (voiceEnabled) setTimeout(() => speak(aiReply), 300);
