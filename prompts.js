...
export default function Settings({
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { icon: "🔒", label: t.changePassword,  sub: t.changePasswordSub },
      ],
    },
    {
      title: t.preferences,
      items: [
        ...
        { icon: "🔊", label: t.voiceEnabled,      sub: `${t.voiceSub} — ${REGIONAL_VOICES[voiceKey]?.name || voiceKey}`, toggle: voiceEnabled, onToggle: () => setVoiceEnabled(!voiceEnabled) },
      ],
    },
    {
      ...
      items: [
        {
          ...
          sub: `${langInfo?.country || ""} — ${t.languageSub}`,
          custom: <LanguageBadge langInfo={langInfo} onChangeLocale={onChangeLocale} />,
        },
      ],
    },
    {
      ...
      items: [
        {
          icon: user?.isPremium ? "⭐" : "🚀",
          label: user?.isPremium ? t.premiumPlan : t.upgradePremium,
    ...
    },
    {
      ...
      items: [
        ...
        { icon: "📋", label: t.termsOfService, action: () => alert(t.termsText) },
  ...
  return (
    <div style={{ animation: "fadeUp 0.4s ease", padding: "24px 0" }}>
      <h2 style={{ fontFamily: "var(--font-main)", fontSize: 22, fontWeight: 700, marginBottom: 20 }}>
        {t.settingsTitle}
      </h2>
      ...
      <div className="glass" style={{ padding: 20, marginBottom: 18, display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 5, display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 10px", borderRadius: 20, fontSize: 10, fontFamily: "var(--font-main)", background: user?.isPremium ? "rgba(99,102,241,0.15)" : "rgba(100,116,139,0.15)", color: user?.isPremium ? "var(--accent)" : "var(--text-muted)" }}>
      ...
      {sections.map((sec) => (
        <div key={sec.title} style={{ marginBottom: 16 }}>
          <p style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 7, fontFamily: "var(--font-main)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", paddingLeft: 4 }}>
            {sec.title}
          </p>
          <div className="glass" style={{ overflow: "hidden", padding: 0 }}>
            {sec.items.map((item, i) => (
              <div
                ...
                style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderBottom: i < sec.items.length - 1 ? "1px solid var(--border)" : "none", cursor: item.action ? "pointer" : "default", transition: "background 0.2s" }}
      ...
      <button
        onClick={onLogout}
        style={{ width: "100%", marginTop: 8, padding: "12px 24px", borderRadius: "var(--radius-sm)", background: "rgba(239,68,68,0.12)", color: "var(--danger)", border: "1px solid rgba(239,68,68,0.2)", fontFamily: "var(--font-main)", fontSize: 14, fontWeight: 500, cursor: "pointer", transition: "all 0.2s" }}
